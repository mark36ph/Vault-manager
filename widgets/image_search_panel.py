...
from image_search import ImageSearchError, download_image_to_project, search_images
...
class ImageSearchPanel(ctk.CTkFrame):
    """Embedded image browser for a project page."""

    def __init__(self, parent, project, project_folder=None):
        super().__init__(parent, fg_color="transparent")
        self.project = project
        self.project_folder = Path(project_folder or project["folder"])
    ...
    def _build(self):
        ...
        self.search_button.grid(row=0, column=4, padx=(0, 15), pady=15)
        ...
        for column in range(2):
            self.results_frame.grid_columnconfigure(
                column, weight=1, uniform="image-result"
            )
        ...
        buttons = ctk.CTkFrame(self.preview_panel, fg_color="transparent")
        buttons.grid(
            row=4, column=0, padx=16, pady=(0, 16), sticky="ew"
        )
    ...
    def _resolve_providers(self, selection):
        ...
        for provider in requested:
            api_key = str(
                self.settings.get("images", key_names[provider], "") or ""
            ).strip()
            (available if api_key else missing).append(provider)
    ...
    def _perform_search(self, query, providers, orientation, missing):
        ...
        results = self._interleave_results(groups)
    ...
    @staticmethod
    def _interleave_results(groups):
        merged = []
    ...
    def _display_results(self, results, providers, missing, errors):
        ...
        message = f"{len(results)} images found from {' and '.join(providers)}."
        extras = []
        if missing:
    ...
    def _load_image_async(self, url, label, target_size, is_preview):
        threading.Thread(
    ...
    def _fetch_image(self, url, label, target_size, is_preview):
    ...
    def _perform_download(self, result):
        try:
            image_path = download_image_to_project(result, self.project_folder)
        except ImageSearchError as exc:
            self.after(0, lambda: self._show_download_error(str(exc)))
            return
        except Exception as exc:
