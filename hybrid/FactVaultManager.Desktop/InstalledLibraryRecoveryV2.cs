...
public static class InstalledLibraryRecoveryV2
{
    ...
    public static void Run()
    {
        ...
        try
        {
            var known = CandidateDatabasePaths(appDataRoot).ToList();
            var destination = InspectDatabase(Path.Combine(appDataRoot, "data", "factvault.db"));

            if (destination.LibraryRows == 0 && !ContainsUsableSource(known))
                known.AddRange(DeepDiscoverDatabasePaths(appDataRoot));

            _ = Run(appDataRoot, known);
        }
        ...
        IEnumerable<string> sourceDatabasePaths)
    {
        ...
        // A previous Build 55 recovery marker is deliberately ignored here. Build 56
        // only stops once the installed database itself contains real Library/history rows.
    ...
    private static bool ContainsUsableSource(IEnumerable<string> paths)
    {
        ...
        {
            var snapshot = InspectDatabase(path);
            if (snapshot.LibraryRows > 0)
                return true;
        }
        return false;
    }
    ...
    private static IEnumerable<string> CandidateDatabasePaths(string appDataRoot)
    {
        ...
        foreach (var path in MarkerDatabasePaths(appDataRoot))
        {
            ...
            if (full.Length > 0 && yielded.Add(full))
                yield return full;
        }
        ...
        foreach (var root in CommonCheckoutRoots(oneDrive).Concat(NamedDocumentRoots(Path.Combine(oneDrive, "Documents"))))
        {
            foreach (var path in DatabaseNamesUnderRoot(root))
            {
                var full = TryFullPath(path);
        ...
        {
            foreach (var path in SafeEnumerateFiles(migrationBackup, maxDepth: 4, maxDirectories: 2_000))
            {
                var full = TryFullPath(path);
                if (full.Length > 0 && yielded.Add(full))
                    yield return full;
    ...
    private static IEnumerable<string> MarkerDatabasePaths(string appDataRoot)
    {
        ...
        {
            var path = Path.Combine(appDataRoot, markerName);
            if (!File.Exists(path))
                continue;
            ...
            try
            {
                marker = JsonNode.Parse(File.ReadAllText(path)) as JsonObject;
            }
            catch (Exception error) when (error is IOException or UnauthorizedAccessException or JsonException)
            {
                Debug.WriteLine($"Could not read recovery marker '{path}': {error.Message}");
            }
            ...
            {
                foreach (var candidate in AncestorDatabasePaths(sourceProjects))
        ...
        if (File.Exists(developmentRootMarker))
        {
            ...
            try
            {
                root = File.ReadAllText(developmentRootMarker).Trim();
            }
            catch (Exception error) when (error is IOException or UnauthorizedAccessException)
            {
                root = "";
                Debug.WriteLine($"Could not read development root marker: {error.Message}");
            }

            if (root.Length > 0)
            {
                foreach (var candidate in DatabaseNamesUnderRoot(root))
                    yield return candidate;
            }
        }
    }
    ...
    private static IEnumerable<string> DeepDiscoverDatabasePaths(string appDataRoot)
    {
        ...
        {
            var root = TryFullPath(item.Root);
            if (root.Length == 0 || !Directory.Exists(root) || !seenRoots.Add(root))
                continue;
            if (IsWithin(root, appDataRoot))
                continue;

            foreach (var file in SafeEnumerateFiles(root, item.MaxDepth, item.MaxDirectories))
            {
                var full = TryFullPath(file);
                if (full.Length == 0 || PathsEqual(full, Path.Combine(appDataRoot, "data", "factvault.db")))
                    continue;
    ...
    private static IEnumerable<(string Root, int MaxDepth, int MaxDirectories)> SearchRoots()
    {
        var documents = Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments);
        if (!string.IsNullOrWhiteSpace(documents))
            yield return (documents, 8, 12_000);

        var desktop = Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory);
        if (!string.IsNullOrWhiteSpace(desktop))
            yield return (desktop, 8, 8_000);

        var oneDrive = Environment.GetEnvironmentVariable("OneDrive") ?? "";
        if (!string.IsNullOrWhiteSpace(oneDrive))
            yield return (oneDrive, 8, 12_000);

        var profile = Environment.GetFolderPath(Environment.SpecialFolder.UserProfile);
        if (!string.IsNullOrWhiteSpace(profile))
            yield return (profile, 5, 15_000);

        var localAppData = Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData);
        if (!string.IsNullOrWhiteSpace(localAppData))
            yield return (localAppData, 4, 8_000);

        var roaming = Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData);
        if (!string.IsNullOrWhiteSpace(roaming))
            yield return (roaming, 4, 8_000);
        ...
        foreach (var drive in drives)
        {
            try
            {
                if (!drive.IsReady || drive.DriveType != DriveType.Fixed)
                    continue;
                if (string.Equals(
                        Path.GetPathRoot(profile),
                        drive.RootDirectory.FullName,
                        StringComparison.OrdinalIgnoreCase))
                    continue;
                yield return (drive.RootDirectory.FullName, 6, 20_000);
            }
            catch
            {
            }
    ...
    private static IEnumerable<string> SafeEnumerateFiles(string root, int maxDepth, int maxDirectories)
    {
    ...
    private static void CopySupplementalManagerFiles(string sourceData, string destinationData)
    {
        ...
        foreach (var name in new[] { "youtube-manager-cache.json" })
        {
            ...
            if (!File.Exists(source))
                continue;
            if (!File.Exists(destination) || new FileInfo(source).LastWriteTimeUtc > new FileInfo(destination).LastWriteTimeUtc)
