...
public sealed class PublicationStateTests
{
    ...
    public void Reconcile_CombinesLegacyQuizHistoryAndDetailedJournal()
    {
        ...
        var youtube = Assert.Single(rows, row => row.Platform == PublicationPlatform.YouTube && row.ContentKind == PublicationContentKind.Quiz);
        ...
        var facebook = Assert.Single(rows, row => row.Platform == PublicationPlatform.Facebook && row.ContentKind == PublicationContentKind.Quiz);
    ...
    public void QuizAndPromo_PublicationsStayIndependentOnSamePlatform()
    {
        ...
        var fullQuiz = Assert.Single(rows, row => row.ContentKind == PublicationContentKind.Quiz);
        var promo = Assert.Single(rows, row => row.ContentKind == PublicationContentKind.Promo);
    ...
    {
        using var fixture = new PublicationFixture();
        fixture.Store.RecordUploaded(21, PublicationPlatform.Facebook, PublicationContentKind.Quiz, "fb-21", "https://www.facebook.com/reel/21");
        fixture.Store.RecordFailure(21, PublicationPlatform.Facebook, PublicationContentKind.Quiz, SocialUploadJournalStep.Comment, "Comment permission denied");
        var failed = Assert.IsType<PublicationStateEntry>(fixture.Store.Get(21, PublicationPlatform.Facebook, PublicationContentKind.Quiz));
        ...
        var cleared = Assert.IsType<PublicationStateEntry>(fixture.Store.Get(21, PublicationPlatform.Facebook, PublicationContentKind.Quiz));
    ...
    private sealed class PublicationFixture : IDisposable
    {
        ...
        public PublicationFixture()
        {
            ...
            command.CommandText = """
                CREATE TABLE quiz_history (
                    id INTEGER PRIMARY KEY, published_on_youtube INTEGER NOT NULL DEFAULT 0, youtube_url TEXT NOT NULL DEFAULT '', youtube_upload_date TEXT NOT NULL DEFAULT '', youtube_privacy TEXT NOT NULL DEFAULT '', youtube_scheduled_for TEXT NOT NULL DEFAULT '', published_on_facebook INTEGER NOT NULL DEFAULT 0, facebook_url TEXT NOT NULL DEFAULT '', facebook_upload_date TEXT NOT NULL DEFAULT '', facebook_scheduled_for TEXT NOT NULL DEFAULT '', published_on_instagram INTEGER NOT NULL DEFAULT 0, instagram_url TEXT NOT NULL DEFAULT '', instagram_upload_date TEXT NOT NULL DEFAULT '');
                CREATE TABLE social_upload_journal (
                    history_id INTEGER NOT NULL, platform TEXT NOT NULL, remote_id TEXT NOT NULL DEFAULT '', remote_url TEXT NOT NULL DEFAULT '', upload_status TEXT NOT NULL DEFAULT 'pending', failed_step TEXT NOT NULL DEFAULT '', last_error TEXT NOT NULL DEFAULT '', updated_at TEXT NOT NULL DEFAULT '', PRIMARY KEY(history_id, platform));
                """;
            command.ExecuteNonQuery();
            Store = new PublicationStateStore(_databasePath);
        }
        public PublicationStateStore Store { get; }
        public void InsertQuizHistory(int id, bool youtubePublished = false, string youtubeUrl = "", string youtubePrivacy = "", string youtubeScheduledFor = "", bool facebookPublished = false, string facebookUrl = "")
        {
            using var connection = Open();
            using var command = connection.CreateCommand();
        ...
        public void InsertJournal(int historyId, string platform, string remoteId, string remoteUrl, string uploadStatus, string failedStep, string lastError)
        {
            using var connection = Open();
            using var command = connection.CreateCommand();
            command.CommandText = """INSERT INTO social_upload_journal(history_id, platform, remote_id, remote_url, upload_status, failed_step, last_error, updated_at) VALUES($historyId, $platform, $remoteId, $remoteUrl, $uploadStatus, $failedStep, $lastError, $updatedAt)""";
            command.Parameters.AddWithValue("$historyId", historyId); command.Parameters.AddWithValue("$platform", platform); command.Parameters.AddWithValue("$remoteId", remoteId); command.Parameters.AddWithValue("$remoteUrl", remoteUrl); command.Parameters.AddWithValue("$uploadStatus", uploadStatus); command.Parameters.AddWithValue("$failedStep", failedStep); command.Parameters.AddWithValue("$lastError", lastError); command.Parameters.AddWithValue("$updatedAt", DateTimeOffset.UtcNow.ToString("O")); command.ExecuteNonQuery();
        }
        ...
        public void Dispose() { SqliteConnection.ClearAllPools(); if (Directory.Exists(_root)) Directory.Delete(_root, recursive: true); }
    }
}
