...
public sealed class AutopilotNeedsYouAlignedPlannerTests
{
    ...
    public void ScreenshotState_ExcludesFutureAndWaitingInstagramPromosFromNeedsYou()
    {
        ...
        var rows = Enumerable.Range(0, 14).Select(index => Row(100 + index, publishAt.AddDays(index), $"Quiz {index + 1}", index == 0 ? "Uploaded" : "Missing", index < 8 ? "Next day" : "Waiting", index == 0 ? "Needs setting" : "Waiting")).ToList();
        var state = new FactburstFullAutopilotState { ReplyDrafts = Enumerable.Range(0, 12).Select(index => new YouTubeReplyDraft { CommentId = $"comment-{index}", VideoId = $"video-{index}", Author = $"Viewer {index}", CommentText = "Quiz comment", Draft = "Thanks for playing!", CreatedAtUtc = new DateTime(2026, 8, 29, 9, index, 0, DateTimeKind.Utc) }).ToList() };
        ...
        Assert.Single(tasks, task => task.Kind == AutopilotAlignedTaskKind.RelatedVideo && task.ActionReady);
    ...
    public void InstagramPromo_AppearsOnlyWhenNextDayAndPostingTimeHasArrived()
    {
        ...
        var beforeDue = AutopilotNeedsYouAlignedPlanner.Build([nextDay, waiting], new FactburstFullAutopilotState(), [], due.AddMinutes(-1));
        var atDue = AutopilotNeedsYouAlignedPlanner.Build([nextDay, waiting], new FactburstFullAutopilotState(), [], due);
        Assert.DoesNotContain(beforeDue, task => task.Kind == AutopilotAlignedTaskKind.InstagramPromo);
        var instagram = Assert.Single(atDue, task => task.Kind == AutopilotAlignedTaskKind.InstagramPromo);
    ...
    {
        var row = Row(9, DateTimeOffset.UtcNow.AddDays(2), "Science Quiz", "Uploaded", "Uploaded", "Set");
