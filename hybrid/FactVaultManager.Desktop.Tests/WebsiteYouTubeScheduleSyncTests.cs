...
public sealed class WebsiteYouTubeScheduleSyncTests
{
    ...
    public void ScheduledYouTubeReleaseWinsOverCallerWebsiteTime()
    {
        ...
        var history = History(privacy: "private", scheduledFor: scheduled.ToString("O"));
        var plan = WebsiteYouTubeSchedulePlanner.Plan(history, now, now.AddMinutes(5));
        var resolved = WebsiteYouTubeSchedulePlanner.ResolvePublishAtOrThrow(history, now.AddMinutes(5), now);
        Assert.NotNull(plan);
    ...
    public void PublicYouTubeVideoIsLiveAndUsesItsUploadTime()
    {
        ...
        var history = History(privacy: "public", uploadDate: uploaded.ToString("O"));
    ...
    public void PrivateOrUnlistedUnscheduledUploadCannotPublishWebsiteEarly(string privacy)
    {
        ...
        var error = Assert.Throws<InvalidDataException>(() => WebsiteYouTubeSchedulePlanner.ResolvePublishAtOrThrow(history, now, now));
    ...
    {
        var now = new DateTimeOffset(2026, 9, 1, 12, 0, 0, TimeSpan.Zero);
        var history = History(privacy: "public", format: "9:16", uploadDate: now.AddHours(-1).ToString("O"));
        Assert.Null(WebsiteYouTubeSchedulePlanner.Plan(history, now));
    }
    ...
    public void WebsiteTimingComparisonUsesOneSecondTolerance()
    {
        ...
        Assert.True(WebsiteYouTubeSchedulePlanner.PublishTimesMatch(expected.AddMilliseconds(500).ToString("O"), expected));
        Assert.False(WebsiteYouTubeSchedulePlanner.PublishTimesMatch(expected.AddSeconds(2).ToString("O"), expected));
    }
    ...
    public void WebsiteSync_RemainsWiredAndUsesExactScheduledSync()
    {
        ...
        Assert.Contains("QueueDeferredShellPhase(InitializeWebsiteYouTubeScheduleSync);", build, StringComparison.Ordinal);
