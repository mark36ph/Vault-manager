...
public sealed class CreateAdvancedUiCleanupTests
{
    ...
    public void Create_steps_use_plain_daily_workflow_labels(string key, string expected) => Assert.Equal(expected, FactburstDailyWorkspaceLayout.CreateStepLabel(key));
    ...
    {
        var troubleshooting = Assert.Single(FactburstDailyWorkspaceLayout.AdvancedGroups, group => group.Collapsed);
