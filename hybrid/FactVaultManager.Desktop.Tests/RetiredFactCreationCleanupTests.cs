...
public sealed class RetiredFactCreationCleanupTests
{
    ...
    [Fact]
    public void Build193_BumpsVersionAndBuildNumber()
    {
        var buildInfo = ReadRepositoryFile("hybrid/FactVaultManager.Desktop/MainShellWindow.BuildInfo.cs");
        var version = ReadRepositoryFile("version.json");
        Assert.Contains("CurrentBuildNumber = 193", buildInfo, StringComparison.Ordinal);
        Assert.Contains("\"build\": 193", version, StringComparison.Ordinal);
        Assert.Contains("\"latest_version\": \"1.0.172\"", version, StringComparison.Ordinal);
    }
