...
public sealed class RetiredFactCreationCleanupTests
{
    ...
    [Fact]
    public void Build192_BumpsVersionAndBuildNumber()
    {
        var buildInfo = ReadRepositoryFile("hybrid/FactVaultManager.Desktop/MainShellWindow.BuildInfo.cs");
        var version = ReadRepositoryFile("version.json");
        Assert.Contains("CurrentBuildNumber = 192", buildInfo, StringComparison.Ordinal);
        Assert.Contains("\"build\": 192", version, StringComparison.Ordinal);
        Assert.Contains("\"latest_version\": \"1.0.171\"", version, StringComparison.Ordinal);
    }
