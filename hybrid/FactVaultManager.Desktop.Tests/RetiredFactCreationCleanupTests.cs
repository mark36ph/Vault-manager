...
public sealed class RetiredFactCreationCleanupTests
{
    ...
    [Fact]
    public void Build191_BumpsVersionAndBuildNumber()
    {
        var buildInfo = ReadRepositoryFile("hybrid/FactVaultManager.Desktop/MainShellWindow.BuildInfo.cs");
        var version = ReadRepositoryFile("version.json");
        Assert.Contains("CurrentBuildNumber = 191", buildInfo, StringComparison.Ordinal);
        Assert.Contains("\"build\": 191", version, StringComparison.Ordinal);
        Assert.Contains("\"latest_version\": \"1.0.170\"", version, StringComparison.Ordinal);
    }
