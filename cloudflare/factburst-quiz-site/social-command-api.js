...
async function verifyAdminSessionCookie(token,siteKey){const parts=String(token).split('.');if(parts.length===2){try{const expected=await hmacSha256(siteKey,parts[0]);const received=fromBase64Url(parts[1]);return constantTimeEqual(expected,received);}catch{return false;}}if(parts.length!==3)return false;const timestamp=Number(parts[0]);const now=Date.now()/1000;const maxAge=30*24*60*60;if(!Number.isInteger(timestamp)||now-timestamp>maxAge||timestamp-now>60)return false;try{const expected=await hmacSha256(siteKey,`factburst-admin-session:${parts[0]}.${parts[1]}`);const received=fromBase64Url(parts[2]);return constantTimeEqual(expected,received);}catch{return false;}}
