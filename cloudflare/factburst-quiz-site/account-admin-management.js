...
export async function handleAdminUserManagementApi(request, env, url) {
  ...
  if (parts.length === 5 && parts[0] === "api" && parts[1] === "admin" && parts[2] === "users" && parts[4] === "friends" && request.method === "DELETE") {
    ...
    await env.DB.prepare(`DELETE FROM site_friendships WHERE (user_a_id = ? AND user_b_id = ?) OR (user_a_id = ? AND user_b_id = ?)`).bind(Math.min(userId, friendId), Math.max(userId, friendId), Math.min(userId, friendId), Math.max(userId, friendId)).run();
...
async function listUsers(db, url) {
  ...
  const result = await db.prepare(`
    SELECT u.id,u.username,u.email,u.email_verified_at,u.status,u.created_at,u.last_login_at,
      u.referral_code, r.username AS referred_by_username,
      COUNT(DISTINCT s.quiz_id) AS quizzes_completed,
      COUNT(DISTINCT f.id) AS friend_count
    FROM site_users u
    LEFT JOIN site_users r ON r.id = u.referred_by_user_id
    LEFT JOIN site_user_scores s ON s.user_id = u.id
    LEFT JOIN site_friendships f ON (f.user_a_id = u.id OR f.user_b_id = u.id) AND f.status = 'accepted'
    WHERE u.username LIKE ? ESCAPE '\\' COLLATE NOCASE OR u.email LIKE ? ESCAPE '\\' COLLATE NOCASE OR COALESCE(u.referral_code,'') LIKE ? ESCAPE '\\' COLLATE NOCASE
...
async function detailUser(db, userId) {
  ...
  const friends = await db.prepare(`
    SELECT other.id, other.username, other.email, other.email_verified_at, f.status, f.created_at
    FROM site_friendships f JOIN site_users other ON other.id = CASE WHEN f.user_a_id = ? THEN f.user_b_id ELSE f.user_a_id END
    WHERE (f.user_a_id = ? OR f.user_b_id = ?) ORDER BY other.username COLLATE NOCASE ASC
  ...
  return json({ user: mapUser(user), friends: (friends.results || []).map(row => ({ id: Number(row.id), username: String(row.username || ""), email: String(row.email || ""), email_verified: Boolean(row.email_verified_at), status: String(row.status || "pending"), created_at: String(row.created_at || "") })), referrals: (referrals.results || []).map(row => ({ id: Number(row.id), username: String(row.username || ""), email: String(row.email || ""), email_verified: Boolean(row.email_verified_at), created_at: String(row.created_at || "") })) });
}
...
async function updateUser(request, env, userId) {
  ...
  if (!username) return json({ error: "Choose a valid username." }, 400);
  if (!email) return json({ error: "Enter a valid email address." }, 400);
  if (password && (password.length < 10 || password.length > 128)) return json({ error: "Password must be 10–128 characters." }, 400);
  ...
  if (duplicate) return json({ error: "That username or email address is already in use." }, 409);
  const emailChanged = emailKey !== String(existing.email_key || "").toLowerCase();
  const statements = [];
  let passwordChanged = false;
  if (password) {
    const pepper = String(env.PASSWORD_PEPPER || "").trim();
    if (pepper.length < 32) return json({ error: "Password security is not configured on the site." }, 503);
  ...
  if (passwordChanged) statements.push(env.DB.prepare("DELETE FROM site_sessions WHERE user_id = ?").bind(userId));
...
async function ensureReferralSchema(db) {
  ...
  if (!names.has("referral_code")) await db.prepare("ALTER TABLE site_users ADD COLUMN referral_code TEXT").run();
  if (!names.has("referred_by_user_id")) await db.prepare("ALTER TABLE site_users ADD COLUMN referred_by_user_id INTEGER").run();
  if (!names.has("referred_at")) await db.prepare("ALTER TABLE site_users ADD COLUMN referred_at TEXT").run();
  ...
  for (const row of missing.results || []) {
    ...
    for (let attempt = 0; attempt < 5 && !code; attempt++) {
      const candidate = `FB-${randomToken(6).replace(/[^A-Za-z0-9]/g, "").slice(0, 8).toUpperCase()}`;
      const taken = await db.prepare("SELECT id FROM site_users WHERE referral_code = ? LIMIT 1").bind(candidate).first();
      if (!taken) code = candidate;
    }
    if (code) await db.prepare("UPDATE site_users SET referral_code = ? WHERE id = ? AND (referral_code IS NULL OR referral_code = '')").bind(code, row.id).run();
  }
}

function mapUser(row) {
  return { id: Number(row.id || 0), username: String(row.username || ""), email: String(row.email || ""), email_verified: Boolean(row.email_verified_at), email_verified_at: row.email_verified_at ? String(row.email_verified_at) : null, status: String(row.status || "active"), created_at: String(row.created_at || ""), last_login_at: String(row.last_login_at || ""), referral_code: String(row.referral_code || ""), referred_by_username: row.referred_by_username ? String(row.referred_by_username) : null, friend_count: Number(row.friend_count || 0), quizzes_completed: Number(row.quizzes_completed || 0) };
}
function randomToken(byteCount) { const bytes = new Uint8Array(byteCount); crypto.getRandomValues(bytes); let binary=""; for (const byte of bytes) binary += String.fromCharCode(byte); return btoa(binary).replace(/\+/g,"-").replace(/\//g,"_").replace(/=+$/g,""); }
