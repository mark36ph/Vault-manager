...
async function perQuizLeaderboard(request, db, slug, url) {
  ...
  if (!length) {
    ...
    return buildResponse(request, db, quiz, result.results || [], null);
  }
...
async function buildResponse(request, db, quiz, rows, length) {
  ...
  if (current?.email_verified_at) {
    const row = length
      ? await db.prepare(`SELECT best_score, question_count AS total, attempts, first_completed_at FROM site_user_quiz_scores_by_length WHERE user_id = ? AND quiz_id = ? AND question_count = ? LIMIT 1`).bind(current.id, quiz.id, length).first()
      : null;
    if (row) {
