...
async function ensureSocialStatsSchema(db){
  await db.prepare(`CREATE TABLE IF NOT EXISTS site_social_stats(
    id INTEGER PRIMARY KEY AUTOINCREMENT,quiz_slug TEXT NOT NULL,platform TEXT NOT NULL,platform_id TEXT NOT NULL DEFAULT '',external_id TEXT NOT NULL DEFAULT '',title TEXT NOT NULL DEFAULT '',url TEXT NOT NULL DEFAULT '',status TEXT NOT NULL DEFAULT 'unknown',scheduled_for TEXT,uploaded_at TEXT,published_at TEXT,views INTEGER NOT NULL DEFAULT 0,likes INTEGER NOT NULL DEFAULT 0,comments INTEGER NOT NULL DEFAULT 0,shares INTEGER NOT NULL DEFAULT 0,reactions INTEGER NOT NULL DEFAULT 0,captured_at TEXT NOT NULL,last_stats_sync TEXT,error_message TEXT NOT NULL DEFAULT '',updated_at TEXT NOT NULL DEFAULT ''
  )`).run();
  ...
  await db.batch([
    ...
    db.prepare(`CREATE TABLE IF NOT EXISTS site_social_upload_history(id INTEGER PRIMARY KEY AUTOINCREMENT,quiz_slug TEXT NOT NULL,platform TEXT NOT NULL,external_id TEXT NOT NULL DEFAULT '',url TEXT NOT NULL DEFAULT '',title TEXT NOT NULL DEFAULT '',status TEXT NOT NULL DEFAULT 'unknown',scheduled_at TEXT,uploaded_at TEXT,published_at TEXT,error_message TEXT NOT NULL DEFAULT '',created_at TEXT NOT NULL)`),
...
async function addColumnIfMissing(db,table,column,definition){const info=await db.prepare(`PRAGMA table_info(${table})`).all();const names=new Set((info.results||[]).map(row=>String(row.name||"")));if(!names.has(column))await db.prepare(`ALTER TABLE ${table} ADD COLUMN ${column} ${definition}`).run();}

async function listSocialStats(db,url){
  const platform=normalizePlatform(url.searchParams.get("platform"));const slug=String(url.searchParams.get("slug")||"").trim().toLowerCase();const includeHistory=url.searchParams.get("history")==="1";const limit=Math.min(Math.max(Number.parseInt(url.searchParams.get("limit")||"100",10),1),500);const where=[];const binds=[];if(platform){where.push("platform=?");binds.push(platform);}if(slug){where.push("quiz_slug=?");binds.push(slug);}const whereSql=where.length?`WHERE ${where.join(" AND ")}`:"";const result=await db.prepare(`SELECT quiz_slug,platform,platform_id,external_id,title,url,status,scheduled_for,uploaded_at,published_at,views,likes,comments,shares,reactions,captured_at,last_stats_sync,error_message,updated_at FROM site_social_stats ${whereSql} ORDER BY COALESCE(updated_at,captured_at) DESC,id DESC LIMIT ?`).bind(...binds,limit).all();const stats=result.results||[];const summary=buildSummary(stats);if(!includeHistory)return json({stats,summary});const historyResult=await db.prepare(`SELECT id,quiz_slug,platform,external_id,url,title,status,scheduled_at,uploaded_at,published_at,error_message,created_at FROM site_social_upload_history ${whereSql} ORDER BY created_at DESC,id DESC LIMIT ?`).bind(...binds,Math.min(limit,200)).all();return json({stats,summary,history:historyResult.results||[]});
}

function buildSummary(stats){const summary={total_published:0,total_scheduled:0,total_failed:0,total_uploading:0,total_pending:0,total_views:0,total_likes:0,total_comments:0,total_shares:0,total_reactions:0,platforms:{youtube:{published:0,scheduled:0,failed:0,views:0,likes:0,comments:0,shares:0,reactions:0},facebook:{published:0,scheduled:0,failed:0,views:0,likes:0,comments:0,shares:0,reactions:0},instagram:{published:0,scheduled:0,failed:0,views:0,likes:0,comments:0,shares:0,reactions:0}}};for(const row of stats){const p=summary.platforms[row.platform]||summary.platforms.instagram;const status=normalizeStatus(row.status);if(status==="published"){summary.total_published++;p.published++;}if(status==="scheduled"){summary.total_scheduled++;p.scheduled++;}if(status==="failed"){summary.total_failed++;p.failed++;}if(status==="uploading")summary.total_uploading++;if(status==="pending")summary.total_pending++;for(const key of ["views","likes","comments","shares","reactions"]){const value=metric(row[key]);summary[`total_${key}`]+=value;p[key]+=value;}}return summary;}

async function upsertSocialStats(request,db){let body;try{body=await request.json();}catch{return json({error:"Request body must be valid JSON."},400);}const records=Array.isArray(body?.stats)?body.stats:[body];if(records.length===0||records.length>100)return json({error:"Send between 1 and 100 social stat records."},400);const now=new Date().toISOString();const statements=[];const historyStatements=[];for(const item of records){const quizSlug=normalizeSlug(item?.quiz_slug);const platform=normalizePlatform(item?.platform);if(!quizSlug)return json({error:"Each record needs a valid quiz_slug."},400);if(!platform)return json({error:"Each record needs YouTube, Facebook, or Instagram as platform."},400);const status=normalizeStatus(item?.status);const externalId=String(item?.external_id||item?.platform_id||"").trim().slice(0,200);const values={externalId,title:String(item?.title||"").trim().slice(0,200),url:String(item?.url||"").trim().slice(0,1000),status,scheduledAt:normalizeDate(item?.scheduled_at??item?.scheduled_for),uploadedAt:normalizeDate(item?.uploaded_at),publishedAt:normalizeDate(item?.published_at),views:metric(item?.views),likes:metric(item?.likes),comments:metric(item?.comments),shares:metric(item?.shares),reactions:metric(item?.reactions),capturedAt:normalizeDate(item?.captured_at)||now,errorMessage:String(item?.error_message||"").trim().slice(0,1000)};const existing=await db.prepare("SELECT quiz_slug,platform,external_id,platform_id,url,title,status,scheduled_for,uploaded_at,published_at,error_message FROM site_social_stats WHERE quiz_slug=? AND platform=? LIMIT 1").bind(quizSlug,platform).first();const changed=!existing||String(existing.status||"")!==values.status||String(existing.external_id||existing.platform_id||"")!==values.externalId||String(existing.url||"")!==values.url||String(existing.error_message||"")!==values.errorMessage||String(existing.scheduled_for||"")!==(values.scheduledAt||"")||String(existing.published_at||"")!==(values.publishedAt||"");if(changed)historyStatements.push(db.prepare(`INSERT INTO site_social_upload_history(quiz_slug,platform,external_id,url,title,status,scheduled_at,uploaded_at,published_at,error_message,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?)`).bind(quizSlug,platform,values.externalId,values.url,values.title,values.status,values.scheduledAt,values.uploadedAt,values.publishedAt,values.errorMessage,now));statements.push(db.prepare(`INSERT INTO site_social_stats(quiz_slug,platform,platform_id,external_id,title,url,status,scheduled_for,uploaded_at,published_at,views,likes,comments,shares,reactions,captured_at,last_stats_sync,error_message,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(quiz_slug,platform) DO UPDATE SET platform_id=excluded.platform_id,external_id=excluded.external_id,title=excluded.title,url=excluded.url,status=excluded.status,scheduled_for=excluded.scheduled_for,uploaded_at=excluded.uploaded_at,published_at=excluded.published_at,views=excluded.views,likes=excluded.likes,comments=excluded.comments,shares=excluded.shares,reactions=excluded.reactions,captured_at=excluded.captured_at,last_stats_sync=excluded.last_stats_sync,error_message=excluded.error_message,updated_at=excluded.updated_at`).bind(quizSlug,platform,values.externalId,values.externalId,values.title,values.url,values.status,values.scheduledAt,values.uploadedAt,values.publishedAt,values.views,values.likes,values.comments,values.shares,values.reactions,values.capturedAt,now,values.errorMessage,now));}await db.batch([...statements,...historyStatements]);return json({ok:true,saved:records.length,captured_at:now,history_events:historyStatements.length});}
...
function isStatsWriterAuthorized(request,env){const supplied=String(request.headers.get("authorization")||"").replace(/^Bearer\s+/i,"").trim();if(supplied.length<16)return false;const dedicated=String(env.SOCIAL_STATS_API_KEY||"").trim();if(dedicated&&supplied===dedicated)return true;const legacy=String(env.TRACKER_API_KEY||"").trim();return Boolean(legacy&&supplied===legacy);}
