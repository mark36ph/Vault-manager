(() => {
  ...
  function shell(){const app=document.querySelector("#admin-app");if(!app)return;let nav=app.querySelector(".admin-sidebar");if(!nav){nav=document.createElement("aside");nav.className="admin-sidebar";nav.setAttribute("aria-label","Admin navigation");app.insertBefore(nav,app.firstElementChild);}if(!nav.hasAttribute("aria-label"))nav.setAttribute("aria-label","Admin navigation");if(!nav.querySelector("a")){nav.innerHTML=`<div class="admin-sidebar-label">Manage</div>${NAV.map(([label,href])=>`<a href="${href}" class="${href===path?"active":""}">${label}</a>`).join("")}`;}}
