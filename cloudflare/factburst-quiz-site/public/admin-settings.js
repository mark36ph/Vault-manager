(() => {
  ...
  if(adsEnabled&&adsClient&&adsLeft&&adsRight&&adsSave){adsSave.addEventListener("click",async()=>{adsSave.disabled=true;setStatus(adsStatus,"Saving…");try{const d=await api("/api/admin/site/ads",{method:"PATCH",headers:{"content-type":"application/json"},body:JSON.stringify({enabled:adsEnabled.checked,client:adsClient.value,left_slot:adsLeft.value,right_slot:adsRight.value})});adsEnabled.checked=!!d.enabled;adsClient.value=d.client||"";adsLeft.value=d.left_slot||"";adsRight.value=d.right_slot||"";setStatus(adsStatus,"Google Ads settings saved.","success");setStatus(adsState,d.active?"Google Ads are active on the public site.":"Google Ads are currently inactive.",d.active?"success":"");}catch(e){setStatus(adsStatus,e.message,"error");}finally{adsSave.disabled=false;}});loadAds();}
