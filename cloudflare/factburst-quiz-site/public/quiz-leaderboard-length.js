(() => {
  ...
  const load = async length => {
    if (!length) return; list.replaceChildren(Object.assign(document.createElement("p"), { className: "leaderboard-message", textContent: "Loading high scores…" }));
    try { const response = await fetch(`/api/quizzes/${encodeURIComponent(slug)}/leaderboard?limit=50&length=${encodeURIComponent(length)}`, { headers: { accept: "application/json" } }); if (!response.ok) throw new Error("Leaderboard unavailable"); render(await response.json()); }
    catch { list.replaceChildren(Object.assign(document.createElement("p"), { className: "leaderboard-message", textContent: "The leaderboard is not available right now. Please try again." })); }
  };
  const sync = () => {
    const length = getLength(); if (!length) return;
    ...
    select.value = String(length); load(length);
  };
