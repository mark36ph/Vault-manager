(() => {
  ...
  function render(stats) {
    ...
    $("social-youtube-views").textContent = number(totals.youtube);
    $("social-facebook-views").textContent = number(totals.facebook);
    $("social-instagram-views").textContent = number(totals.instagram);

    $("social-engagement").textContent = number(totals.engagement);
    const list = $("social-list");
    const empty = $("social-empty");
    list.innerHTML = "";
    empty.classList.toggle("hidden", stats.length !== 0);
    for (const item of stats) {
      ...
      const metrics = platform === "facebook"
        ? `Views ${number(item.views)} · Reactions ${number(item.reactions)} · Comments ${number(item.comments)} · Shares ${number(item.shares)}`
        : `Views ${number(item.views)} · Likes ${number(item.likes)} · Comments ${number(item.comments)}`;
      const date = item.captured_at ? new Date(item.captured_at).toLocaleString() : "—";
      const link = item.url ? `<a href="${esc(item.url)}" target="_blank" rel="noopener">Open post</a>` : "";
