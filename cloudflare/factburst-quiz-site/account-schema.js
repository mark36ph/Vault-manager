...
export const SITE_USER_UPGRADES = [
  ...
  { name: "role", sql: "ALTER TABLE site_users ADD COLUMN role TEXT NOT NULL DEFAULT 'user'" },
  ...
  { name: "last_login_at", sql: "ALTER TABLE site_users ADD COLUMN last_login_at TEXT NOT NULL DEFAULT ''" },
];
...
const AFTER_USER_SCHEMA = [
  ...
  `CREATE TABLE IF NOT EXISTS site_user_scores (user_id INTEGER NOT NULL,quiz_id INTEGER NOT NULL,best_score INTEGER NOT NULL,total INTEGER NOT NULL,attempts INTEGER NOT NULL DEFAULT 1,first_completed_at TEXT NOT NULL,last_completed_at TEXT NOT NULL,PRIMARY KEY (user_id, quiz_id),FOREIGN KEY (user_id) REFERENCES site_users(id) ON DELETE CASCADE)`,
  ...
  `CREATE TABLE IF NOT EXISTS site_settings (key TEXT PRIMARY KEY,value TEXT NOT NULL DEFAULT '',updated_at TEXT NOT NULL)`,
