...
async function requireAdmin(request,env){const cookie=String(request.headers.get("cookie")||"");const match=cookie.match(/(?:^|;\s*)factburst_admin_session=([^;]+)/);if(match){const row=await env.DB.prepare("SELECT expires_at FROM site_admin_sessions WHERE token_hash=? AND expires_at>? LIMIT 1").bind(await sha256(match[1]),new Date().toISOString()).first();if(row)return {ok:true};}const key=String(request.headers.get("authorization")||"").replace(/^Bearer\s+/i,"").trim();const expected=String(env.SITE_ADMIN_KEY||"").trim();if(key&&expected&&key===expected)return {ok:true};return {ok:false,response:json({error:"Administrator access required."},401)};}
