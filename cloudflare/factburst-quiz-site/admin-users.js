...
import { normalizeRole } from "./site-controls.js";
...
async function queueGeneration(request,db){
  ...
  const categories=Array.isArray(body?.categories)?body.categories.map(value=>String(value||"").trim()).filter(Boolean).slice(0,30):[];
  const count=Math.min(10,Math.max(1,Number.parseInt(body?.quizzes_per_quiz||body?.quizzes_per_run||1,10)));
...
async function listUsers(db,url){const search=String(url.searchParams.get("search")||"").trim();const limit=Math.min(Math.max(Number(url.searchParams.get("limit")||50),1),100);const pattern=`%${search.replace(/[%_]/g,"\\$&")}%`;const where=search?"WHERE u.username LIKE ? ESCAPE '\\\\' OR u.email LIKE ? ESCAPE '\\\\' OR u.referral_code LIKE ? ESCAPE '\\\\'":"";const bindings=search?[pattern,pattern,pattern,limit]:[limit];const result=await db.prepare(`SELECT u.id,u.username,u.email,u.email_verified_at,u.status,u.role,u.created_at,u.last_login_at,u.referral_code,(SELECT COUNT(*) FROM site_friendships f WHERE (f.user_a_id=u.id OR f.user_b_id=u.id) AND f.status='accepted') AS friends_count,(SELECT r.username FROM site_users r WHERE r.id=u.referred_by_user_id LIMIT 1) AS referred_by FROM site_users u ${where} ORDER BY u.created_at DESC,u.id DESC LIMIT ?`).bind(...bindings).all();return json({users:(result.results||[]).map(publicUser)});}
...
async function getUser(db,userId){const user=await db.prepare(`SELECT u.id,u.username,u.email,u.email_verified_at,u.status,u.role,u.suspended_at,u.suspension_reason,u.created_at,u.last_login_at,u.referral_code,(SELECT r.username FROM site_users r WHERE r.id=u.referred_by_user_id LIMIT 1) AS referred_by FROM site_users u WHERE u.id=? LIMIT 1`).bind(userId).first();if(!user)return json({error:"User not found."},404);const friends=await db.prepare(`SELECT f.id,f.status,f.created_at,CASE WHEN f.user_a_id=? THEN f.user_b_id ELSE f.user_a_id END AS friend_id,CASE WHEN f.user_a_id=? THEN b.username ELSE a.username END AS friend_username,CASE WHEN f.user_a_id=? THEN b.email ELSE a.email END AS friend_email FROM site_friendships f JOIN site_users a ON a.id=f.user_a_id JOIN site_users b ON b.id=f.user_b_id WHERE f.user_a_id=? OR f.user_b_id=? ORDER BY f.created_at DESC`).bind(userId,userId,userId,userId,userId).all();const referred=await db.prepare("SELECT id,username,email,created_at,email_verified_at,status,role FROM site_users WHERE referred_by_user_id=? ORDER BY created_at DESC").bind(userId).all();return json({user:publicUser(user),friends:friends.results||[],referred_users:referred.results||[]});}
async function updateUser(request,env,userId){let body;try{body=await request.json();}catch{return json({error:"Request body must be valid JSON."},400);}const existing=await env.DB.prepare("SELECT id,username_key,email_key,email_verified_at,status,role FROM site_users WHERE id=? LIMIT 1").bind(userId).first();if(!existing)return json({error:"User not found."},404);const username=normalizeUsername(body?.username);const email=normalizeEmail(body?.email);const password=String(body?.password||"");if(!username)return json({error:"Choose a username 3–24 characters long."},400);if(!email)return json({error:"Enter a valid email address."},400);if(password&&(password.length<10||password.length>128))return json({error:"Password must be between 10 and 128 characters."},400);const usernameKey=username.toLowerCase();const emailKey=email.toLowerCase();const duplicate=await env.DB.prepare("SELECT id FROM site_users WHERE id<>? AND (username_key=? OR email_key=?) LIMIT 1").bind(userId,usernameKey,emailKey).first();if(duplicate)return json({error:"That username or email is already in use."},409);const emailChanged=emailKey!==String(existing.email_key||"").toLowerCase();const role=normalizeRole(body?.role===undefined?existing.role:body.role);const statusValue=body?.status===undefined?String(existing.status||"active").toLowerCase():String(body.status||"").trim().toLowerCase();if(!["active","suspended"].includes(statusValue))return json({error:"Choose Active or Suspended for the account status."},400);const suspensionReason=statusValue==="suspended"?String(body?.suspension_reason||"").trim().slice(0,500):"";const now=new Date().toISOString();const statements=[];const baseArgs=[username,usernameKey,email,emailKey,emailChanged?1:0,role,statusValue,statusValue==="suspended"?now:null,suspensionReason,userId];if(password){const pepper=String(env.PASSWORD_PEPPER||"").trim();if(pepper.length<32)return json({error:"Password security is not configured."},503);const salt=randomToken(18);const hash=await derivePasswordHash(password,salt,pepper,PASSWORD_POLICY.iterations);statements.push(env.DB.prepare(`UPDATE site_users SET username=?,username_key=?,email=?,email_key=?,email_verified_at=CASE WHEN ? THEN NULL ELSE email_verified_at END,role=?,status=?,suspended_at=?,suspension_reason=?,password_hash=?,password_salt=?,password_iterations=?,password_scheme=? WHERE id=?`).bind(...[...baseArgs.slice(0,10),hash,salt,PASSWORD_POLICY.iterations,PASSWORD_POLICY.scheme,userId]));statements.push(env.DB.prepare("DELETE FROM site_sessions WHERE user_id=?").bind(userId));}else statements.push(env.DB.prepare(`UPDATE site_users SET username=?,username_key=?,email=?,email_key=?,email_verified_at=CASE WHEN ? THEN NULL ELSE email_verified_at END,role=?,status=?,suspended_at=?,suspension_reason=? WHERE id=?`).bind(...baseArgs));if(emailChanged)statements.push(env.DB.prepare("DELETE FROM site_email_verifications WHERE user_id=?").bind(userId));if(statusValue==="suspended"&&!password)statements.push(env.DB.prepare("DELETE FROM site_sessions WHERE user_id=?").bind(userId));await env.DB.batch(statements);return getUser(env.DB,userId);}
...
function publicUser(user){return{id:Number(user.id),username:String(user.username||""),email:String(user.email||""),email_verified:Boolean(user.email_verified_at),email_verified_at:user.email_verified_at||null,status:String(user.status||"active"),role:normalizeRole(user.role),created_at:user.created_at||null,last_login_at:user.last_login_at||null,referral_code:String(user.referral_code||""),referred_by:user.referred_by||null,friends_count:Number(user.friends_count||0),suspended_at:user.suspended_at||null,suspension_reason:String(user.suspension_reason||"")};}
function normalizeUsername(value){const v=String(value||"").trim().replace(/\s+/g," ");return v.length>=3&&v.length<=24&&/^[A-Za-z0-9 ._-]+$/.test(v)?v:"";}
function normalizeEmail(value){const v=String(value||"").trim().toLowerCase();return v.length<=254&&/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(v)?v:"";}
function randomToken(bytes=32){const data=new Uint8Array(bytes);crypto.getRandomValues(data);return base64UrlEncode(data);}
function randomCode(length=8){const alphabet="ABCDEFGHJKLMNPQRSTUVWXYZ23456789";const data=new Uint8Array(length);crypto.getRandomValues(data);return Array.from(data,value=>alphabet[value%alphabet.length]).join("");}
...
async function requireAdmin(request,env){const configured=String(env.SITE_ADMIN_KEY||"").trim();const header=String(request.headers.get("authorization")||"");const bearer=header.replace(/^Bearer\s+/i,"").trim();if(configured&&bearer&&constantTimeEqual(bearer,configured))return{ok:true};const cookie=cookieValue(request,ADMIN_SESSION_COOKIE);if(cookie&&configured&&await verifyAdminSessionCookie(cookie,configured))return{ok:true};return{ok:false,response:json({error:"Administrator authentication required."},401)};}
async function verifyAdminSessionCookie(token,siteKey){const parts=String(token).split(".");if(parts.length===2){try{const expected=await hmacSha256(siteKey,parts[0]);const received=fromBase64Url(parts[1]);return constantTimeEqualBytes(expected,received);}catch{return false;}}if(parts.length!==3)return false;const timestamp=Number(parts[0]);const now=Date.now()/1000;if(!Number.isInteger(timestamp)||now-timestamp>ADMIN_SESSION_SECONDS||timestamp-now>60)return false;try{const expected=await hmacSha256(siteKey,`factburst-admin-session:${parts[0]}.${parts[1]}`);const received=fromBase64Url(parts[2]);return constantTimeEqualBytes(expected,received);}catch{return false;}}
...
function cookieValue(request,name){const header=request.headers.get("cookie")||"";for(const part of header.split(";")){const separator=part.indexOf("=");if(separator<0)continue;if(part.slice(0,separator).trim()===name)return part.slice(separator+1).trim();}return"";}
...
function constantTimeEqualBytes(a,b){if(a.length!==b.length)return false;let diff=0;for(let i=0;i<a.length;i++)diff|=a[i]^b[i];return diff===0;}
