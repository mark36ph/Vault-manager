...
async function requireAdmin(request,env){const header=String(request.headers.get("authorization")||"");const bearer=header.startsWith("Bearer ")?header.slice(7).trim():"";if(bearer&&await verifySiteAdminKey(bearer,env))return{ok:true};const cookie=cookieValue(request,ADMIN_SESSION_COOKIE);if(cookie){const hash=await sha256(cookie);const row=await env.DB.prepare("SELECT token_hash,expires_at FROM site_admin_sessions WHERE token_hash=? LIMIT 1").bind(hash).first();if(row&&Date.parse(String(row.expires_at||""))>Date.now())return{ok:true};}return{ok:false,response:json({error:"Administrator authentication required."},401)};}
async function verifySiteAdminKey(value,env){const configured=String(env.SITE_ADMIN_KEY||"").trim();return Boolean(configured&&value&&constantTimeEqual(value,configured));}
