...
async function listUsers(db,url){const search=String(url.searchParams.get("search")||"").trim();const limit=Math.min(Math.max(Number(url.searchParams.get("limit")||50),1),100);const pattern=`%${search.replace(/[%_]/g,"\\$&")}%`;const where=search?"WHERE u.username LIKE ? ESCAPE '\\\\' OR u.email LIKE ? ESCAPE '\\\\' OR u.referral_code LIKE ? ESCAPE '\\\\'":"";const bindings=search?[pattern,pattern,pattern,limit]:[limit];const result=await db.prepare(`SELECT u.id,u.username,u.email,u.email_verified_at,u.status,u.role,u.created_at,u.referral_code,(SELECT COUNT(*) FROM site_friendships f WHERE (f.user_a_id=u.id OR f.user_b_id=u.id) AND f.status='accepted') AS friends_count,(SELECT r.username FROM site_users r WHERE r.id=u.referred_by_user_id LIMIT 1) AS referred_by FROM site_users u ${where} ORDER BY u.created_at DESC,u.id DESC LIMIT ?`).bind(...bindings).all();return json({users:(result.results||[]).map(publicUser)});}
