...
export async function handleAdminUsersApi(request, env, url) {
  ...
  try {
    ...
    return json({ error: "The user management request could not be completed." }, 500);
  }
}
...
async function listUsers(db, url) {
  ...
  const result = await db.prepare(`SELECT u.id,u.username,u.email,u.email_verified_at,u.status,u.created_at,u.referral_code,
    ...
    FROM site_users u ${where} ORDER BY u.created_at DESC, u.id DESC LIMIT ?`).bind(...bindings).all();
...
async function createUser(request, env) {
  ...
  const verifyEmail = body?.verify_email === true || body?.verify_email === "true";
  ...
  const duplicate = await env.DB.prepare("SELECT id FROM site_users WHERE username_key=? OR email_key=? LIMIT 1").bind(usernameKey,emailKey).first();
  ...
  const referralCode = await uniqueReferralCode(env.DB);
  const verifiedAt = verifyEmail ? now : null;
  const result = await env.DB.prepare(`INSERT INTO site_users
    (username,username_key,email,email_key,email_verified_at,password_hash,password_salt,password_iterations,password_scheme,status,referral_code,created_at,last_login_at)
    VALUES (?,?,?,?,?,?,?,?,?,'active',?,?,?)`).bind(username,usernameKey,email,emailKey,verifiedAt,hash,salt,PASSWORD_POLICY.iterations,PASSWORD_POLICY.scheme,referralCode,now,now).run();
  const userId = Number(result.meta?.last_row_id || 0);
  if (!userId) return json({ error: "The user could not be created." }, 500);
...
async function uniqueReferralCode(db) {
  for (let attempt = 0; attempt < 10; attempt++) {
    ...
    const exists = await db.prepare("SELECT id FROM site_users WHERE referral_code=? LIMIT 1").bind(code).first();
...
async function getUser(db, userId) {
  const user = await db.prepare(`SELECT u.id,u.username,u.email,u.email_verified_at,u.status,u.suspended_at,u.suspension_reason,
    u.created_at,u.last_login_at,u.referral_code,(SELECT r.username FROM site_users r WHERE r.id=u.referred_by_user_id LIMIT 1) AS referred_by
    FROM site_users u WHERE u.id=? LIMIT 1`).bind(userId).first();
  if (!user) return json({ error: "User not found." }, 404);
  const friends = await db.prepare(`SELECT f.id,f.status,f.created_at,
    ...
    FROM site_friendships f JOIN site_users a ON a.id=f.user_a_id JOIN site_users b ON b.id=f.user_b_id
    WHERE f.user_a_id=? OR f.user_b_id=? ORDER BY f.created_at DESC`).bind(userId,userId,userId,userId,userId).all();
  const referred = await db.prepare("SELECT id,username,email,created_at,email_verified_at,status FROM site_users WHERE referred_by_user_id=? ORDER BY created_at DESC").bind(userId).all();
  return json({ user: publicUser(user), friends: friends.results || [], referred_users: referred.results || [] });
}
...
async function updateUser(request, env, userId) {
  ...
  const username = normalizeUsername(body?.username); const email = normalizeEmail(body?.email); const password = String(body?.password || "");
  if (!username) return json({ error: "Choose a username 3–24 characters long." }, 400);
  if (!email) return json({ error: "Enter a valid email address." }, 400);
  if (password && (password.length < 10 || password.length > 128)) return json({ error: "Password must be between 10 and 128 characters." }, 400);
  const usernameKey = username.toLowerCase(); const emailKey = email.toLowerCase();
  ...
  if (duplicate) return json({ error: "That username or email is already in use." }, 409);
  const emailChanged = emailKey !== String(existing.email_key || "").toLowerCase(); const statements = [];
  if (password) {
    const pepper = String(env.PASSWORD_PEPPER || "").trim(); if (pepper.length < 32) return json({ error: "Password security is not configured." }, 503);
    const salt = randomToken(18); const hash = await derivePasswordHash(password, salt, pepper, PASSWORD_POLICY.iterations);
  ...
  } else statements.push(env.DB.prepare(`UPDATE site_users SET username=?,username_key=?,email=?,email_key=?,email_verified_at=CASE WHEN ? THEN NULL ELSE email_verified_at END WHERE id=?`).bind(username,usernameKey,email,emailKey,emailChanged?1:0,userId));
  if (emailChanged) statements.push(env.DB.prepare("DELETE FROM site_email_verifications WHERE user_id=?").bind(userId));
  await env.DB.batch(statements); return getUser(env.DB, userId);
}

async function verifyUser(db, userId) {
  const exists = await db.prepare("SELECT id FROM site_users WHERE id=? LIMIT 1").bind(userId).first(); if (!exists) return json({ error: "User not found." },404);
  const now = new Date().toISOString(); await db.prepare("UPDATE site_users SET email_verified_at=? WHERE id=?").bind(now,userId).run(); await db.prepare("DELETE FROM site_email_verifications WHERE user_id=?").bind(userId).run(); return getUser(db,userId);
}
async function unlinkFriend(request, db, userId) {
  let body; try { body = await request.json(); } catch { return json({ error: "Request body must be valid JSON." },400); }
  const friendshipId = Number(body?.friendship_id || 0); if (!friendshipId) return json({ error: "Friendship id is required." },400);
  ...
  if (!result.meta?.changes) return json({ error: "Friendship not found." },404); return json({ unlinked:true });
}
...
async function requireAdmin(request,env){
  const expected=String(env.SITE_ADMIN_KEY||"").trim(); if(!expected)return {ok:false,response:json({error:"Website publishing is not enabled yet."},503)};
  const cookie=String(request.headers.get("cookie")||""); const match=cookie.split(";").map(value=>value.trim()).find(value=>value.startsWith(`${ADMIN_SESSION_COOKIE}=`));
  if(match){const token=match.slice(`${ADMIN_SESSION_COOKIE}=`.length);if(await verifyAdminSessionToken(token,expected))return {ok:true};}
  const key=String(request.headers.get("authorization")||"").replace(/^Bearer\s+/i,"").trim();if(key&&key===expected)return {ok:true};
...
async function verifyAdminSessionToken(token,siteKey){
  if(!token||token.length>MAX_TOKEN_LENGTH)return false; const parts=token.split(".");if(parts.length!==3)return false; const timestamp=Number(parts[0]);if(!Number.isInteger(timestamp))return false;
  const now=Date.now()/1000;if(now-timestamp>ADMIN_SESSION_SECONDS||timestamp-now>60)return false;
  const expected=await hmacSha256(siteKey,`factburst-admin-session:${parts[0]}.${parts[1]}`);let received;try{received=fromBase64Url(parts[2]);}catch{return false;}
  if(expected.length!==received.length)return false;let difference=0;for(let i=0;i<expected.length;i++)difference|=expected[i]^received[i];return difference===0;
}
